...
def str_by_twos(iterable):
    """ Iterate over a string by consecutive pairs. Used for stack energy
        calculations and maybe should be local there, but there may be other
        areas of use. Looked for an equivalent function in itertools and didn't
...
class EnergyModel(object):
    """ TODO: (JMS 4/8/16) Add docstring.
        ...
        John SantaLucia Jr., "A unified view of polymer, dumbbell, and
        ...
        Hatim T. Allawi and John SantaLucia Jr., "Thermodynamics and NMR of
        Internal G-T Mismatches in DNA", Biochemistry 36 (1997): 10581-10594.

        We use the following convention for storage, which is similar to the
        one used by Nupack (J. N. Zadeh, C. D. Steenberg, J. S. Bois,
            B. R. Wolfe, M. B. Pierce, A. R. Khan, R. M. Dirks, N. A. Pierce.
            NUPACK: analysis and design of nucleic acid systems. J Comput Chem,
            32:170-173, 2011.)
    ...
    def stack_energy(self, sequence_1, sequence_2):
        """ Computes the energy of a single base pair stack, or a helical
            region consisting only of stacks.

            sequence_1:   5'-XY-3'     "XY"
            sequence_2:   3'-WZ-5'     "ZW"

            Computes the nearest neighbor energy of the above stack. If the
            sequences are more than 2 bases, e.g.:

            sequence_1:   5'-GGCCA-3'    "GGCCA"
            sequence_2:   3'-CCGGT-5'    "TGGCC"

            it will compute the NN energies for each stack in turn and return
            the sum. In this case, it would computethe energy of the four
            stacks below and return the sum:
            ...
            Args:
                sequence_1 (str): base identifiers for top strand, 5' to 3'
                sequence_2 (str): base identifiers for bottom strand, 5' to 3'
            Returns:
            ...
            temperature (37 deg C) vs what the reference dG has. It doesn't
            appear to be a rounding/floating point issue, but something is off.
            However, it does seem to agree roughly with Nupack calculations
        ...
        for top, bot in zip(str_by_twos(sequence_1),
                            str_by_twos(reversed(sequence_2))
                            ):
            ...
            # the division by 1000.0 is the unit change back to kcal/mol
            dG_37_pair = dH_pair - self.temperature_in_K * dS_pair / 1000.0
    ...
    def melting_temperature(self,
                            ...
                            scaffold_conc=10e-9,
                            sodium_conc=1.0,
                            magnesium_conc=20e-3
                            ):
        """
        Units: dH is in kcal/mol, dS is in cal/mol (note difference). All
        ...
        conc_derived_term = (BOLTZMANN_CONSTANT
                             * math.log(eff_staple_conc_at_tm)
                             )
        # important unit conversion for dS
        denominator = dS / 1000.0 + conc_derived_term
...
# TODO: Convert the below code to a unit test on the energy computation?
# (JMS 11/21/16)
