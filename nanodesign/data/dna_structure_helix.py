...
""" This module defines the classes used to define a structure helix of a DNA
    structure.

    A structure helix is a region in a DNA structure that forms a cylindrical
    structural element. It can be composed of one or two DNA strands defined by
    the set of scaffold and staple bases assigned to the helix.

    A helix can be considered to be a container for storing scaffold and staple
    bases. The position of a base in a helix is given by an integer between 0
    and N-1, where N is the maximum number of bases staple or scaffold,
    assigned to it.
...
from .parameters import DnaPolarity
...
class DnaStructureHelix(object):
    """ This class stores information for a DNA structure helix.
        ...
        Attributes:
            load_order (int): The order (0-based) the helix was processed.
                This is needed when writing a caDNAno file.
            dna_parameters (DnaParameters): The DNA parameters to use when
                creating the 3D geometry for the design.
            end_coordinates (NumPy 2x3 ndarray[float]): The coordinates at the
                ends of the helix.
            end_frames (NumPy 3x3x2 ndarray[float]): The coordinate frames at
                the ends of the helix.
            helix_axis_frames (NumPy 3x3xN ndarray[float]): The coordinate
                frames of base nodes along the helix axis,
                where N is the number of bases of the caDNAno virtual helix.
            helix_axis_coords (NumPy Nx3 ndarray[float]): The coordinates of
                base nodes along the helix axis, where N is the
                number of bases of the caDNAno virtual helix. The elements of
                the array are sorted according to their position in the helix.
            helix_connectivity (List[DnaHelixConnection]): The list helices
                that this helix is connected to.
            id (int): Helix ID. This should be the same IDs used for a base
            helix number (i.e. the DnaBase.h ID).
            ...
            possible_scaffold_crossovers (List[(DnaStructureHelix,int,NumPy 3x1
                array[float])]): The list of possible scaffold crossovers for
                this helix. Each entry in the list is a 3-tuple giving the
                helix it crosses over to (object pointer), the helix position
                where the crossover occurs and the helix axis coodinates.
            possible_staple_crossovers (List[(DnaStructureHelix,int,NumPy 3x1
                array[float])]): The list of possible staple crossovers for
                this helix. Each entry in the list is a 3-tuple giving the
                helix it crosses over to (object pointer), the helix position
                where the crossover occurs the helix axis coodinates.
            scaffold_bases (List[DnaBase]): The list of helix scaffold bases.
            scaffold_polarity (DnaPolarity): The polarity of the scaffold.
            scaffold_pos (Dict[int,DnaBase]: The dict mapping helix positions
                to scaffold bases.
            staple_bases (List[DnaBase]): The list of helix staple bases.
            staple_pos (Dict[int,DnaBase]: The dict mapping helix positions to
                staple bases.

        The geometry of a helix is determined by the coordinates of base nodes
        along its axis. The coordinates and reference frames of the bases
        defined for the helix are references into the helix_axis_coords and
        helix_axis_frames arrays.
    """

    # TODO (DaveP) We need to remove the references to caDNAno lattice-based
    # nformation.
    def __init__(self, load_order, id, scaffold_polarity, helix_axis_coords,
        ...
        """ Initialize a DnaStructureHelix object.
            ...
            Arguments:
                ...
                helix_axis_coords (NumPy Nx3 ndarray[float]): The coordinates
                    of base nodes along the helix axis.
                helix_axis_frames (NumPy 3x3xN ndarray[float]): The coordinate
                    frames base nodes alonge the helix axis.
                scaffold_bases (List[DnaBase]): The list of scaffold bases
                    defined for the helix.
                scaffold_coords (NumPy Nx3 ndarray[float]): The scaffold DNA
                    helix nucleotide coordinates.
                staple_bases (List[DnaBase]): The list of staple bases defined
                    for the helix.
                staple_coords (NumPy Nx3 ndarray[float]): The staple DNA helix
    ...
    def set_coordinates(self, helix_axis_coords, helix_axis_frames,
                        scaffold_coords, staple_coords
                        ):
        """ Set the helix axis coordinates and frames, and DNA helix nucleotide
            coordinates.
    ...
    def has_base_pos(self, pos):
        """ Check if the helix has bases at the given helix position.
            ...
            Returns two booleans, each set to True if there is a staple base
            and a scaffold base at the given position.
        """
    ...
    def build_base_pos_maps(self):
        """ Create maps for base positions.
            ...
            The max/min positions for staple and scaffold bases are also
            calculated.
                self.min_staple_pos
                self.max_staple_pos
    ...
    def add_maximal_staple_bases(self):
        """ Add bases for the maximal staple set.
            ...
            New bases are created and added to the helix at locations not
            occupied by bases.
            The helix locations to add bases are detemined by the max/min
            positions of the
            helix scaffold bases.
        """
        self._logger.debug(
            "================= add maximal staple bases %d ==================="
            % self.id
        )
    ...
    def add_maximal_staple_crossovers(self):
        ...
        self._logger.debug(
            "=============== add maximal staple crossovers %d ================"
            % self.id
        )
        self._logger.debug("Scaffold polarity %s" % self.scaffold_polarity)
            ...
            if has_staple and has_scaffold and (pos in to_helix.staple_pos):
                ...
                if five_prime:
                    ...
                    if (base.p + 1) in crossover_pos:
                        ...
                        to_base.down = base
                self._logger.debug(
                    "Crossover base at pos %d to helix %d"
                    % (base.p, to_helix.id)
                )
    ...
    def apply_xform(self, xform):
        ...
        self._logger.debug(
            "Xform center (%g %g %g)" % (center[0], center[1], center[2])
        )
        self._logger.debug(
            "Xform translation (%g %g %g)"
    ...
    def compute_design_crossovers(self, dna_structure):
        """ Determine the scaffold and staple crossovers in the designed
            structure.
        """
        self._logger.debug(
            "=========== compute design cross-overs p helix num %d ==========="
            % self.lattice_num
        )
        self._logger.debug("Helix polarity %s " % self.scaffold_polarity)
        self._logger.debug(
            "Helix connectivity: %d " % len(self.helix_connectivity)
        )
        ...
        for connection in self.helix_connectivity:
            ...
            self._logger.debug(
                "Number of staple bases: %d" % len(self.staple_bases)
            )
            last_crossover = None
                ...
                for base in base_list:
                    ...
                    if base.up is not None:
                        if (base.up.h != base.h) and (base.up.h == num):
                            ...
                            connection.crossovers.append(crossover)
            self._logger.debug(
                ">>> added %d crossovers " % len(connection.crossovers)
            )
    ...
    def remove_bases(self, base_list):
        """ Remove a list of bases from the helix.
            ...
            This function is used to remove bases after a design file has been
            processed.
        ...
        self._logger.debug(
            "Number of bases removed %d"
            % (len(self.staple_bases) - len(remaining_staple_bases))
        )
        self._logger.debug(
            "Number of bases remaining %d" % len(remaining_staple_bases)
        )
        self.staple_bases = remaining_staple_bases
        for base in self.staple_bases:
            self._logger.debug(
                "Base ID %d  h %d  p %d" % (base.id, base.h, base.p)
            )
    ...
    def process_base_deletes(self):
        """ Process bases flagged for deletion.
            ...
            This function is used to remove bases flagged for deletion
            (DnaBase.num_deletion) in the design file. It is called when the
            design file is being processed. Bases are removed from the helix
    ...
    def insert_bases(self, insert_bases):
        """ Insert a list of bases into the helix lists of scaffold and staple
            bases.
            ...
            if insert_base.is_scaf:
                for i, base in enumerate(self.scaffold_bases):
                    if base.p == insert_base.p:
                        self._logger.debug(
                            "Insert scaffold base at position %d"
                            % insert_base.p
                        )
                        self._logger.debug(
                ...
                for i, base in enumerate(self.staple_bases):
                    if base.p == insert_base.p:
                        self._logger.debug(
                            "Insert staple base at position  %d"
                            % insert_base.p
                        )
                        self._logger.debug(
                            "    base coord        %s" % str(base.coordinates)
    ...
    def regenerate_coordinate_arrays(self):
        """ Regenerate the coordinate arrays after a change in the base lists.
            ...
            This will regenerate the coordinate arrays after deletions or
            insertions. The base helix position cannot be used to sort the
            arrays because inserted bases will have the same position. The
            arrays will therefore be sorted by base distance from the beginning
            helix coordinates.
        """
        self._logger.debug(
            "=============== regenerate_coordinate_arrays %d ================="
            % self.lattice_num
        )

        # Define the origin and axis used to calculate the distance along helix
...
class DnaHelixConnection(object):
    """ This class stores information for a pair of connected helices.
        ...
        Attributes:
            ...
            direction (NumPy 3x1 adarray[float]): The unit vector in the
                direction of the connection.
            crossovers (List[DnaHelixCrossover]): The list of crossovers
    ...
    def _compute_direction(self):
        """ Compute the unit vector in the direction of the adjacent helix.
            ...
            This function uses helix axes to compute the direction so it can be
            used for lattice or off-lattice geometries.
            However, for lattice-based geometries the directions can be
            calculated implicitly using a lookup table.
        """
        # Get the first helix axis and a point on that axis from staple bases.
        ...
        axis1 = [helix1.end_frames[0, 2, 0],
                 helix1.end_frames[1, 2, 0],
                 helix1.end_frames[2, 2, 0]
                 ]
        ...
        axis2 = [helix2.end_frames[0, 2, 0],
                 helix2.end_frames[1, 2, 0],
                 helix2.end_frames[2, 2, 0]
                 ]
...
class DnaHelixCrossover(object):
    """ This class stores information for a crossover between two helices.
        ...
        Attributes:
            ...
            helix_connection (DnaHelixConnection): The connection object
                defining the crossover helix.
