...
""" This module is used to store information for a DNA strand.
    A DNA strand is a continuous chain of nucleotides. It can be either a
    scaffold of a staple.
"""
import random
import sys
import numpy as np
...
class DnaStrand(object):
    ...
    Attributes:
        base_id_list (Dict): The location of each strand base within list of
        base IDs making up the strand. The dictionary maps base IDs to an index
        into tour[].
        ...
        helix_list (Dict): The list of helices the strand passes through. The
        dictionary maps helix IDs to DnaStructureHelix objects.
        icolor (int): The strand color as an integer. The integer color can be
        used as an ID to group staple strands.
        id (int): The strand ID.
        insert_seq (List[string]): The list of sequence letters inserted into
            this strand.
        is_circular (bool): If True then the strand is circular, returning to
            its starting postion.
    ...
    def __init__(self, id, dna_structure, is_scaffold, is_circular, tour):
        """ Initialize a DnaStrand object.
            ...
            Arguments:
                id (int): The strand ID.
                dna_structure (DnaStructure): The DNA structure this strand
                    belongs to.
                is_scaffold (bool): If True then the strand is a scaffold
                    strand.
                is_circular (bool): If True then the strand is circular,
                    eturning to its starting postion.
                tour (List[DnaBase]): The list of base objects making up the
                    strand.
    ...
    def get_base_index(self, base):
        ...
        if base.id not in self.base_id_list:
            sys.stderr.write(
                "[strand::get_base_index] **** WARNING: base %d not found in \
                    strand %d.\n"
                % (base.id, self.id)
            )
            return None
