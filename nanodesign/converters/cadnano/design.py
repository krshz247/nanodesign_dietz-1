...
""" This module is used to store data read from caDNAno DNA origami design
    JSON files.
"""
import logging
...
from ...data.lattice import SquareLattice, HoneycombLattice


class CadnanoDesign(object):
    """ The CadnanoDesign class stores data read from a caDNAno origami design
        JSON file.
    """
    ...
    def calculate_possible_crossovers(self):
        """ Calculate the possible crossover positions for both scaffold and
            staples.

            The method for computing cross-ovrs is based on that from the
            caDNAno part.py file.
        """
        self._logger.debug(
        ...
        self._logger.debug(
            ">>> lattice type: %s "
            % CadnanoLatticeType.names[self.lattice_type]
        )
        ...
        for vhelix in self.helices:
            ...
            for neighbor, lut in zip(neighbor_helices, luts_neighbor):
                ...
                for pts, st in zip(lut, strand_types):
                    ...
                    for pt in pts:
                        self._logger.debug(
                            ">>>>>> pt: %s   st: %s" % (str(pt), stype)
                        )
                        self._logger.debug(
                            ">>>>>> base_range: %s " % (str(base_range))
                        )
                        for i, j in product(base_range, pt):
                            index = i + j
                            if index < num_bases:
                                if st == CadnanoStrandType.SCAFFOLD:
                                    vhelix.possible_scaffold_crossovers.append(
                                        (neighbor, index)
                                    )
                                else:
                                    vhelix.possible_staple_crossovers.append(
                                        (neighbor, index)
                                    )
                                self._logger.debug(
                                    "    add i: %4d  j: %4d  index: %4d"
                                    % (i, j, index)
                                )
...
class CadnanoVirtualHelix(object):
    """ This class stores data for a caDNAno virtual helix.
        ...
        Attributes:
            ...
            num (int): The index of the virtual helix. This is the row number
                in the caDNAno 2D design diagram.
            col (int): The column index of the virtual helix in a lattice.
            row (int): The row index of the virtual helix in a lattice.
            possible_staple_crossovers (list[(CadnanoVirtualHelix,int)]: The
                list of possible staple crossovers to neighboring helices.
            possible_scaffold_crossovers (list[(CadnanoVirtualHelix,int)]: The
                list of possible scaffold crossovers to neighboring helices.
    """
...
class CadnanoBase():
    """ This class stores data for a caDNAno base, either staple or scaffold.
        ...
        The data here is equivalent to each 4-tuple [V_0,b_0,V_1,b_1] entry
        stored in the caDNAno JSON 'scaf' and 'stap' arrays.

        Attributes:
            initial_strand (int): id of the initial virtual helix the base
                connects to.
            initial_base (int): id of the initial base.
            final_strand (int): id of the final virtual helix the base connects
                to.
