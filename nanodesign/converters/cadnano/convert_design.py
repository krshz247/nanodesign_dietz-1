...
""" This module is used to create a DNA structure from a caDNAno DNA origami
    design file.

    The virtual helices from the CadnanoDesign object are used to create a list
    of DnaStructureHelix objects. DnaBase objects are created from the scaffold
    and staple bases defined for each virtual helix and stored in the
    appropriate DnaStructureHelix object. A DnaStructure object object is
    created for the design. It stores the list of DnaStructureHelix objects and
    a list of all the bases defined for the design.

    This code is initially based on a direct translation of the set MATLAB
    scripts to convert a caDNAno design to a CanDo .cndo file from Mark Bathe's
    Laboratory for Computational Biology & Biophysics at MIT.
"""
...
from .utils import (generate_coordinates, get_start_coordinates_angle,
                    vrrotvec2mat, deg2rad, bp_interp
                    )

from ...data.base import DnaBase
from ...data.dna_structure import DnaStructure
from ...data.dna_structure_helix import DnaStructureHelix
...
class CadnanoConvertDesign(object):
    """ The CadnanoConvertDesign class is used to create a DNA structure from a
        caDNAno DNA origami design.

        Attributes:
            base_id (int): The current base ID used when creating a DnaBase
                object.
            base_map (OrderedDict[Tuple]): The dict that stores DnaBase objects
                using the tuple (helix_num,helix_pos) as a key.
            dna_parameters (DnaParameters): The DNA parameters to use when
                creating the 3D geometry for the design.
            dna_structure (DnaStructure): The DnaStructure object.
            staple_colors (List[StapleColor]: The list of caDNAno staple strand
                colors.
            Dhp_skips (Set[Tuple[int, int]]): Set containing the helix.id and
                base, position tuple for each skip
    """
    ...
    def _add_staple_color(self, staple_color, vhelix_num):
        """ Add a caDNAno staple color for the given virtual helix.
            ...
            Arguments:
                staple_color (List[int]): The caDNAno color definition
                    consisting of a list of two integer
    ...
    def _get_base_index(self, num, pos, strand_type):
        """ Create a tuple (helix_num,helix_pos,strand_type) used as a key to
            store bases in the base_map dict.

            Arguments:
                num (int): The base virtual helix number.
                pos (int): The base virtual helix position.
                strand_type (StrandType): The type of strand scaffold or staple
                    the base is part of.

            Returns The tuple (helix_num,helix_pos,strand_type) used to
                uniquely identify a base.
         """
        base_index = (int(num), int(pos), int(strand_type))
        return base_index

    def _get_base(self, base_index):
        """ Get a base from the base_map dict. If the base is not in the dict
            then create and add it.

            Arguments:
                base_index (Tuple): The tuple (helix_num,helix_pos,strand_type)
                    used to uniquely identify a base.
    ...
    def create_structure(self, design, modify=False):
        """ Create a DNA structure from a caDNAno DNA origami design.
            ...
            Arguments:
                ...
                modify (bool): If true then modify the caDNAno DNA origami
                    design for inserts and deletes.

            Returns:
                DnaStructure: The DNA structure object containing topological
                    and geometric information for the caDNAno DNA origami
                    design model.

            The virtual helices from the CadnanoDesign object are design are
            used to create a list of DnaStructureHelix objects. DnaBase objects
            are created from the scaffold and staple bases defined for each
            virtual helix and stored in the appropriate DnaStructureHelix
            object. The global list of DnaBase objects are stored in the
            self.base_map dict.
        """

        self._logger.info(
            "Distance between adjacent helices %g "
        ...
        # Set the bases up and down attributes pointing to terminal bases to
        # point to None.
        # These scaffold and staple terminal bases are automatically added when
        # adding caDNAno virtual helix bases.
        ...
        for bindex, base in self.base_map.items():
            ...
            if print_dna_model_base_map:
                self._logger(
                    "%4d: bindex %12s  base id %4d  up %4s  down %4s  across \
                        %4s  h %4d  p %4d  scaf %d"
                    % (n, bindex, base.id, up, down,
                       across, base.h, base.p, base.is_scaf
                       )
                )
            n += 1
        ...
        if self._logger.getEffectiveLevel() == logging.DEBUG:
            ...
            self._logger.debug(
                "size of base_connectivity: %d " % len(base_connectivity)
            )
            for i in range(0, num_bases):
                ...
                across = base.across.id if base.across else -1
                self._logger.debug(
                    "%4d  id %4d  h %4d  p %4d  up %4d  down %4d  across %4d  \
                        scaf %d"
                    % (i, base.id, base.h, base.p,
                       up, down, across, base.is_scaf
                       )
                )
        ...
        if (modify):
            ...
            if print_base_connectivity:
                self._logger.info(
                    "Size of topology after deletes %d"
                ...
                for i, base in enumerate(base_connectivity):
                    ...
                    across = base.across.id if base.across else -1
                    self._logger.info(
                        "%4d: base id %4d  up %4s  down %4s  across %4s  h \
                            %4d  p %4d  scaf %d"
                        % (i, base.id, up, down, across,
                           base.h, base.p, base.is_scaf
                           )
                    )
        ...
        if (modify):
            ...
            if print_base_connectivity:
                self._logger.info(
                    "Size of topology after inserts %d"
                    % len(base_connectivity)
                )
                self._logger.info(
                ...
                for i, base in enumerate(base_connectivity):
                    ...
                    across = base.across.id if base.across else -1
                    self._logger.info(
                        "%4d: base id %4d  up %4s  down %4s  across %4s  \
                            h %4d  p %4d  scaf %d"
                        % (i, base.id, up, down, across,
                           base.h, base.p, base.is_scaf
                           )
        ...
        # Create a DnaStructure object to store base connectivty and helices.
        name = "dna structure"
        ...
        if self._logger.getEffectiveLevel() == logging.DEBUG:
            for strand in strands:
                self._logger.debug(
                    "Strand %4d  bases %4d  scaf %6s  start helix %4d  pos %4d"
                    % (strand.id, len(strand.tour), strand.is_scaffold,
        ...
        # add set of skips to dna_strucute
        self.dna_structure.Dhp_skips = self.Dhp_skips
    ...
    def _delete_bases(self, helices, base_connectivity):
        ...
        for helix in helices:
            ...
            for base in deleted_bases:
                self.Dhp_skips.add((base.h, base.p))
    ...
    def _insert_bases(self, helices, base_connectivity):
        """ Insert bases into helices and base_connectivity[].
            ...
            Arguments:
                helices(List[DnaStructureHelix]): The list of helices for the
                    structure.
                base_connectivity (List[DnaBase]): The list of DNA bases for
                    the structure.

           The DnaBase.num_inserts attribute determines the number of bases
           inserted at that base. Bases are inserted in the 3' direction.
        """
        self._logger.debug(
            "==================== insert bases ===================="
        )
        ...
        for curr_base in base_inserts.values():
            ...
            self._logger.debug("-------------------------------------------")
            self._logger.debug(
                "Current base      ID %3d  h %3d  p %3d  (%6.2f %6.2f %6.2f)"
                % (curr_base.id, curr_base.h, curr_base.p,
            ...
            if curr_base.up:
                ...
                self._logger.debug(
                    "Current base up   ID %3d  h %3d  p %3d  \
                        (%6.2f %6.2f %6.2f) "
                    % (up_base.id, up_base.h, up_base.p,
                       coords[0], coords[1], coords[2],
                       )
                )
            if curr_base.down:
                ...
                self._logger.debug(
                    "Current base down ID %3d  h %3d  p %3d  \
                        (%6.2f %6.2f %6.2f) "
                    % (down_base.id, down_base.h, down_base.p,
                       coords[0], coords[1], coords[2],
                       )
                )
            ...
            if curr_across is not None:
                ...
                # Make sure that the 3'-neighbor of the current base is to the
                # right.
                ...
                if (np.linalg.norm(tmp_a - y_up_vec) < 1e-10):
                    ...
                    # If curr_base is not a preferred base then swap with its
                    # across base.
                ...
                # Reference axis e3 points to the left.
                    ...
                    # If curr_base is a preferred base then swap with its
                    # across base.
            ...
            if curr_across is not None:
                base_id = self._insert_bases_dsDNA(
                    curr_base, base_id, helix_axis,
                    new_bases, base_connectivity,
                )
    ...
    def _insert_bases_ssDNA(self, curr_base, base_id, helix_axis,
                            new_bases, base_connectivity
                            ):
        """ Insert a number of bases into ssDNA.
            ...
            Arguments:
                ...
                base_id (int): The current base ID. This is incremented to set
                    new insert base IDs.
                helix_axis (NumPy 3 ndarray[float]): The helix axis.
                new_bases (Dict): The map of helix IDs to lists of new insert
                    bases.
                base_connectivity (List[DnaBase]): The list of DNA bases for
                the structure.
            Returns base_id.

            New bases are inserted in the 3' direction for curr_base. The new
            DnaBase objects are storedin new_bases{} by helix ID and appended
            to base_connectivity[].
        """
        ...
        [insert_coords, insert_frames] = bp_interp(
            curr_coords, curr_frame,
            next_coords, next_frame,
            num_inserts,
        )
        last_base = None
        self._logger.debug(
            "Current coords %g %g %g"
        ...
        for i in range(0, num_inserts):
            ...
            self._logger.debug(
                "Insert %d coords %g %g %g"
                % (i, insert_coords[i, 0], insert_coords[i, 1],
                   insert_coords[i, 2],
                   )
            )
            base.h = curr_base.h
    ...
    def _insert_bases_dsDNA(self, curr_base, base_id,
                            helix_axis, new_bases, base_connectivity,
                            ):
        """ Insert a number of bases into dsDNA.
            ...
            New bases are inserted in the 3' direction for curr_base. The new
            DnaBase objects are stored in new_bases{} by helix ID and appended
            to base_connectivity[].
        """
        # Rise between two neighboring base-pairs (nm).
        dist_bp = self.dna_parameters.base_pair_rise
        ...
        [insert_coords, insert_frames] = bp_interp(
            curr_coords, curr_frame,
            next_coords, next_frame, num_inserts
        )
        ...
        for i in range(num_inserts):
            ...
            base_id += 1
            self._logger.debug(
                "Add two bases  Base1 id %d   Base 2 id %d "
                % (base1.id, base2.id)
            )
    ...
    def _create_structure_topology_and_geometry(self, design):
        ...
        for i, vhelix in enumerate(vhelices):
            ...
            self._logger.debug(
                "Number of scaffold bases %d "
                % len(scaffold_bases)
            )
            self._logger.debug(
                "Number of staple bases %d "
                % len(staple_bases)
            )
            if self._logger.getEffectiveLevel() == logging.DEBUG:
                s = ""
            ...
            # Generate the helix axis coordinates and frames, and DNA helix
            # nucleotide coordinates.
            axis_coords, axis_frames, scaffold_coords, staple_coords = \
                generate_coordinates(
                    self.dna_parameters, lattice_type, row, col,
                    num, scaffold_bases, staple_bases
                )

            # Create a dna structure object that stores the helix information.
            structure_helix = DnaStructureHelix(
                i, num, scaffold_polarity, axis_coords, axis_frames,
    ...
    def _create_single_helix(self, vhelix, lattice_type):
        """ Create the geometry for a single cadnano virtual helix.
            ...
            Returns:
                scaffold_bases (List[DnaBase]): The list of scaffold bases
                    defined for the helix.
                staple_bases (List[DnaBase]): The list of staple bases defined
                    for the helix.
        """
        ...
        for i in range(0, num_bases):
            ...
            exists_in_scaffold = (
                (current_scaffold.initial_strand >= 0)
                or (current_scaffold.final_strand >= 0)
            )
            if exists_in_scaffold:
                base = self._add_base(
                    StrandType.SCAFFOLD, current_scaffold, StrandType.STAPLE,
                    current_staple, helix_pos, num
                )
            ...
            exists_in_staple = (
                (current_staple.initial_strand >= 0)
                or (current_staple.final_strand >= 0)
            )

            if exists_in_staple:
                base = self._add_base(
                    StrandType.STAPLE, current_staple, StrandType.SCAFFOLD,
                    current_scaffold, helix_pos, num
                )
    ...
    def _add_base(self, base_type, base, paired_base_type,
                  paired_base, helix_pos, helix_num):
        """ Create a base from a cadnano base for a given location in a
            virtual helix.

            Arguments:
                base_type (StrandType): The type of helix strand, SCAFFOLD or
                    STAPLE, the cadnano base is part of.
                base (CadnanoBase): The cadnano base.
                paired_base_type (StrandType): The type of helix paired strand,
                    SCAFFOLD or STAPLE, the cadnano paired base is part of.
        ...
        is_watson_crick_neighbor = (
            (paired_base.initial_strand >= 0)
            or (paired_base.final_strand >= 0)
        )
        if is_watson_crick_neighbor:
            base_index = self._get_base_index(
    ...
    def _calculate_staple_ends(self, strands):
        """ Find the start helix and position for the staple strands in the
            structure.

            Arguments:
                strands (List[DnaStrand]): The list a DnaStrand objects.

            Returns a Dict mapping 2-tuples (start helix, start position) to
            strand ID.
        """
        staple_ends = {}
        for strand in strands:
    ...
    def _set_possible_crossovers(self, design):
        ...
        scaffold_crossovers = []
        staple_crossovers = []
        structure_helices_coord_map = \
            self.dna_structure.structure_helices_coord_map
        for vhelix in design.helices:
            ...
            self._logger.debug(
                "            num staple cross-overs: %d "
                % len(staple_crossovers)
            )
            shelix = structure_helices_coord_map[(row, col)]
            for cross_vh, index in staple_crossovers:
                self._logger.debug(
                    "            staple cross-over: %d, %d "
                    % (cross_vh.num, index)
                )
                cross_sh = structure_helices_coord_map[(cross_vh.row,
                                                        cross_vh.col)
                                                       ]
            ...
            self._logger.debug(
                "            num scaffold cross-overs: %d "
                % len(scaffold_crossovers)
            )
            for cross_vh, index in scaffold_crossovers:
                self._logger.debug(
                    "            scaffold cross-over: %d,%d "
                    % (cross_vh.num, index)
                )
                cross_sh = structure_helices_coord_map[(cross_vh.row,
                                                        cross_vh.col)
    ...
    def _set_strands_colors(self, strands):
        """ Set the color for staple strands.
            ...
            Strands may not have colors assigned to them. If they do then they
            have bothan RGB and integer representation. The integer
            representation can be used as an ID to group staple strands by
            functionality.
        """
        for strand in strands:
            ...
            for staple_color in self.staple_colors:
                is_corerct_color = (
                    (staple_color.vhelix_num == base.h)
                    and (staple_color.vhelix_pos == base.p)
                )
                if is_corerct_color:
                    strand.color = staple_color.rgb
                    strand.icolor = staple_color.color

    def set_sequence_from_name(self, dna_structure,
                               modified_structure, seq_name,
                               ):
        """ Set the sequence information for the staple and scaffold strands
            using a known origami vector sequence name.

            Cadnano seems to start assigning the sequence at virtual helix 0.
            If ordered_traverse=True then use the cadnano method.

            If the structure has not been modified with insertions and
            deletions then we will need to use its insertions and deletions
            information (at the base level) to selectively set its sequence.

            Arguments:
                modified_structure (bool): If true then the structure has been
                    modified for insertions and deletions.
                seq_name (string): The name of the sequence as defined in the
                    dna_sequence_data dictionary.
        """
            ...
            if (ordered_traverse):
                ...
                for j in range(0, len(tour)):
                    ...
                    if ((base.h == min_vh) and (base.p < min_p)):
                        ...
                        start_index = j
                self._logger.debug(
                    "start_index %d  min_vh %d  min_p %d ",
                    start_index, min_vh, min_p
                )
                ...
                for j in range(0, len(tour)):
                    ...
                    base.seq = letter
                    self._logger.debug(
                        "base id %d  vh %d  pos %d  up %d  down %d  across \
                            %d  seq %s",
                        base.id, base.h, base.p, up_base, down_base,
                        across_base, base.seq,
                    )
                ...
                for i in range(0, len(strand.tour)):
                    ...
                    if (not modified_structure):
                        if (base.num_deletions != 0):
                            self._logger.debug("deleted base: id %d  vh %d  \
                                pos %d", base.id, base.h, base.p)
                            letter = 'N'
                        elif (base.num_insertions != 0):
                            self._logger.debug("inserted base: id %d  vh %d  \
                                pos %d", base.id, base.h, base.p)
                    ...
                    base.seq = letter
                    self._logger.debug(
                        "base id %d  vh %d  pos %d  seq %s",
                        base.id, base.h, base.p, base.seq
                    )
    ...
    def set_sequence(self, dna_structure, modified_structure, sequence):
        """ Set the sequence information for the staple and scaffold strands.
            ...
            The caDNAno csv file contains sequence information reflecting the
            insertions and deletions of a design. If a structure has had bases
            inserted and deleted then its sequence can be set directly from the
            sequences in the csv file. If the structure has not been modified
            with insertions and deletions then we will need to use its
            insertions and deletions information (at the base level) to set
            selectively set its sequence from the sequences in the csv file.

            Arguments:
                modified_structure (bool): If True then the structure has been
                    modified with deletions and insertions.
                sequence (DnaSequence): A list of DnaSequence objects
                    representing the sequences for staple or scaffold strands.
        """
        strands = dna_structure.strands
    ...
    class StapleColor(object):
        ...
        def __init__(self, staple_color, vhelix_num):
            ...
            # print("[DnaStapleColor] red %d  green %d  blue %d "
            # % (red, green, blue))
