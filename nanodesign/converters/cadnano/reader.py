...
import logging
import re
from .common import CadnanoLatticeType, CadnanoJsonFields
...
class CadnanoReader(object):
    ...
    def read_json(self, file_name):
        ...
        Args:
            file_name (string): The name of a caDNAno DNA origami design JSON
                file to read.

        Returns:
            CadnanoDesign: A CadnanoDesign object containing the information
                parsed from the input caDNAno DNA origami design JSON file.
        """
        # read the json data from the file. make sure to expand the path first
        # so we can use relative paths or ~ expansion
        import os.path
    ...
    def read_csv(self, file_name):
        ...
        Args:
            file_name (string): The name of a caDNAno DNA origami design CSV
                file to read.

        Returns:
            list (DnaSequence): A list of DnaSequence objects representing the
                sequence for staple or scaffold strands.
        """
    ...
    def parse_json_data(self, json_data):
        ...
        Args:
            json_data: The data read from a caDNAno DNA origami design JSON
                file.
        """
        design = CadnanoDesign()
        num_bases = len(
            json_data[CadnanoJsonFields.VSTRANDS][0][CadnanoJsonFields.SCAF]
        )
        ...
        if ((num_bases % 21 == 0) and (num_bases % 32) == 0):
            ...
            lattice_type = CadnanoLatticeType.honeycomb
        self._logger.info(
            "Lattice type %s " % CadnanoLatticeType.names[lattice_type]
        )
        ...
        for json_helix in json_data[CadnanoJsonFields.VSTRANDS]:
            ...
            self._logger.debug(
                "Number of insertions %d " % insertions.count(-1)
            )
            ...
            is_empty = (
                (scaffold.count([-1, -1, -1, -1])
                 + staples.count([-1, -1, -1, -1])
                 )
                == (len(scaffold) + len(staples))
            )
            if is_empty:
                continue
            ...
            for json_base in scaffold:
                ...
                base = CadnanoBase(
                    initial_strand, initial_base, final_strand, final_base
                )
            ...
            self._logger.debug(
                "Number of scaffold bases %d " % num_scaffold_bases
            )
            self._logger.debug(
                "Number of deletions %d " % deletions.count(-1)
            )
            ...
            max_row_json = max(
                max_row_json, int(json_helix[CadnanoJsonFields.ROW]) + 1
            )
            max_col_json = max(
                max_col_json, int(json_helix[CadnanoJsonFields.COL]) + 1
            )
            num_helix += 1
        ...
        self._logger.info(
            "Number of scaffold bases read %d " % num_scaffold_bases)
        self._logger.info("Maximum number of lattice rows %d " % max_row_json)
        self._logger.info(
            "Maximum number of lattice columns %d " % max_col_json)
        return design
