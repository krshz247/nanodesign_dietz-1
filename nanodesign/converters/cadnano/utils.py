...
""" This module is contains various functions used to create a DNA structure
    from a caDNAno DNA origami design file.
"""
import numpy as np
...
def generate_coordinates(dna_parameters, lattice_type, row, col, helix_num,
                         scaffold_bases, staple_bases):
    """ Generate the axis coordinates, axis reference frames, and nucleotide
        coordinate for the virtual helix.
        ...
        Arguments:
            ...
            scaffold_bases (List[DnaBase]): The list of scaffold bases for this
                helix.
            staple_bases (List[DnaBase]): The list of staple bases for this
                helix.

        Returns:
            axis_coords (NumPy Nx3 ndarray[float]): The coordinates of base
                nodes along the helix axis.
            axis_frames (NumPy 3x3xN ndarray[float]): The coordinate frames of
                base nodes alonge the helix axis.
            scaffold_coords ((NumPy Nx3 ndarray[float]): The coordinates of
                nucleotides along the DNA helix.
            staple_coords ((NumPy Nx3 ndarray[float]): The coordinate of
                nucleotides along the DNA helix.

        The helix coordinates and reference frames are generated for all of the
        virtural helix positions that contain a base. The coordinates and
        refereance frames are also set for scaffold and staple bases.
    """
    # half the distance between the axes of adjacent DNA helices.
    r_strand = dna_parameters.helix_distance / 2.0
    # radius of DNA helices (nm)
    r_helix = dna_parameters.helix_radius
    ...
    scaf_local = r_helix * np.array([cos(deg2rad(180 - ang_minor / 2)),
                                     sin(deg2rad(180 - ang_minor / 2)),
                                     0.0]).transpose()
    stap_local = r_helix * np.array([cos(deg2rad(180 + ang_minor / 2)),
                                     sin(deg2rad(180 + ang_minor / 2)),
                                     0.0]).transpose()

    # Set the helix start coordinates.
    init_coord, init_ang = get_start_coordinates_angle(
        dna_parameters, lattice_type, row, col, helix_num
    )
    ...
    for i, base in enumerate(scaffold_bases):
        ...
        scaffold_coords[i, :] = (axis_coords[j, :]
                                 + np.dot(axis_frames[:, :, j], scaf_local)
                                 )
    ...
    for i, base in enumerate(staple_bases):
        ...
        base.coordinates = axis_coords[j]
        base.ref_frame = axis_frames[:, :, j]
        staple_coords[i, :] = (axis_coords[j, :]
                               + np.dot(axis_frames[:, :, j], stap_local)
                               )
...
def get_start_coordinates_angle(dna_parameters, lattice_type, row, col,
                                helix_num):
    ...
    # Set the helix start coordinates, based on helix (row,col), and frame
    # orientation, based on helix number (polarity).
...
def vrrotmat2vec(R):
    ...
    x = ((m21 - m12)
         / sqrt(pow(m21 - m12, 2) + pow(m02 - m20, 2) + pow(m10 - m01, 2))
         )
    y = ((m02 - m20)
         / sqrt(pow(m21 - m12, 2) + pow(m02 - m20, 2) + pow(m10 - m01, 2))
         )
...
def bp_interp(dnode_1, triad_1, dnode_2, triad_2, n):
    """ Interpolate the position (dnode) and orientation (triad) between two
        base pairs.
    ...
    for i in range(0, n):
        dnode_interp[i, :] = ((dnode_1 * (n + 1 - (i + 1))
                              + dnode_2 * (i + 1))
                              / (n + 1)
                              )
        angle = theta * (i + 1) / (n + 1)
