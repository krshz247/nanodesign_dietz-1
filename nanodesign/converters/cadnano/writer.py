...
class CadnanoWriter(object):
    ...
    def _get_vstrand_info(self, dna_structure):
        """ Get virtual helix information for the design.
            ...
            In caDNAno all the virtual helices are the same size (i.e. the
            same number of base positions). The lists of helix bases for staple
            and scaffold strands may contain fewer bases than the virtual helix
            size so we create an array of empty helix positions ([-1,-1,-1,-1])
            for the caDNAno virtual helix size and fill from the base lists.
        """
        self._logger.info(
            "Number of helices %d " % len(dna_structure.structure_helices_map)
        )
        vstrands_info = []

        # Create a map of helix nums to the list of strands that start in that
        # helix. Used for writing color information.
        self._logger.debug(
            "=================== map helix nums to strands ==================="
        )
        ...
        # Create a map of the helices by ID so we can reproduce the order in
        # which they were read in from the original caDNAno file.
        helix_map = {}
        for helix in dna_structure.structure_helices_map.values():
            helix_map[helix.load_order] = helix

        # Get the base connectivity, loops and skips for the staple and
        # scaffold strands.
        self._logger.debug(
            "=================== get the base connectivity ==================="
        )
        for load_order in sorted(helix_map):
            ...
            self._logger.debug(
                "Helix num %d  num staple bases %d   num scaffold bases %d"
                % (helix.id, len(staple_bases), len(scaffold_bases))
            )
            ...
            self._logger.info(
                "Helix load order %d  num %d  row %d  col %d"
    ...
    def _get_staple_colors(self, strands):
        """ Get the array of staple colors for the strands originating in a
            given helix.

            Arguments:
                strands (List[DnaStrand]): The list of strands originating in a
                    given helix.

            Returns the list of staple colors, staple_colors[].

            In caDNAno staple colors are defined as an array of two-entry
            elements containing the staple starting position in the virtual
            helix and an integer color.
        """
        staple_colors = []
        for strand in strands:
            pos = strand.tour[0].p  # Strand starting position.
            color = (int(255 * strand.color[0]) << 16
                     | int(255 * strand.color[1]) << 8
                     | int(255 * strand.color[2])
                     )
            staple_colors.append([pos, color])
        return staple_colors

    def _get_base_info(self, helix_size, base_list):
        """ Get the array defining caDNAno base information from the list of
            bases.
            ...
            Create an array of empty helix positions ([-1,-1,-1,-1]) for the
            caDNAno virtual helix and fill it in at the positions given by the
            bases in the base list (DnaBase.p).
        """
