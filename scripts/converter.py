...
# Define the map between file formats and the functions that read files in that
# format.
converter_read_map = {ConverterFileFormats.CADNANO: 'read_cadnano_file'}

# Define the map between file formats and the functions that write files in
# that format.
converter_write_map = {
...
def parse_args():
    ...
    parser.add_argument(
        "-dbg", "--debug", help="set modules debugging logger"
    )
    parser.add_argument(
        "-hd", "--helixdist", help="distance between DNA helices"
    )
    parser.add_argument(
        "-if", "--informat", help="input file format: cadnano"
    )
    parser.add_argument(
        "-i", "--infile", help="input file"
    )
    parser.add_argument(
        "-is", "--inseqfile", help="input sequence file"
    )
    parser.add_argument(
        "-isn", "--inseqname", help="input sequence name"
    )
    parser.add_argument(
        "-m", "--modify",
        help="create DNA structure using the deleted/inserted bases given in a\
              cadnano design file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-of", "--outformat",
        help="output file format: cadnano, viewer, cando, cif, pdb, simdna, \
              structure, topology"
    )
    parser.add_argument(
        "-s", "--staples", help="staple operations"
    )
    parser.add_argument(
...
def main():
    ...
    if args.outformat is None:
        ...
        logger.error(
            "Unknown output file format given \'%s\'" % args.outformat
        )
        error_flag = True
    else:
